...
jq --arg rootid "$ROOT_ID" --arg rid "$REGISTRY_ID" --arg bid "$BUCKET_ID" --arg fid "$FLOW_ID" ' .revision.clientId |= $rootid | .component.versionControlInformation.bucketId |= $bid | .component.versionControlInformation.flowId |= $fid| .component.versionControlInformation.registryId |= $rid' $PG > temp.json
curl -X POST -H "Content-Type: application/json" --data @$TEMP http://nifi-worker:8080/nifi-api/process-groups/$ROOT_ID/process-groups
#rm $TEMP